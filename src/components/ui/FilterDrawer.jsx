      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-[var(--surface)] shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-l border-[var(--border)] flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div key={idx} className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${draftFilters[field.keyFrom] && draftFilters[field.keyTo] && draftFilters[field.keyFrom] === draftFilters[field.keyTo] ? 'bg-emerald-500 text-white shadow-md' : 'bg-[var(--surface-hover)] hover:bg-emerald-500/10 text-[var(--text-muted)] hover:text-emerald-500 border border-[var(--border)]'}`}
                    >
                      Today
                    </button>
                    <button 
        ...
        <div className="p-6 border-t border-[var(--border)] bg-[var(--surface)] shrink-0">
          <div className="flex gap-3">
            <button 
              onClick={onReset}
              className="flex-1 py-3.5 px-4 bg-[var(--surface-hover)] text-[var(--text-muted)] hover:text-rose-500 font-bold rounded-xl border border-[var(--border)] transition-colors text-xs uppercase tracking-wider"
            >
              Reset
            </button>
            <button 
